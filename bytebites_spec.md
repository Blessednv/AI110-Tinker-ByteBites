...
Based on the feature request, the four core classes the system must model are:

1. **Customer** — represents a user with a name and purchase history
2. **MenuItem** — represents a single food item with name, price, category, and popularity rating
3. **Menu** — represents the full collection of food items with filtering capability
4. **Order** — represents a transaction that holds selected items and computes the total cost